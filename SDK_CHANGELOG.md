...
## Version Comparison: 2.0.0 → 3.0.0

---

## Version 3.0.0 (Current)

**Release Focus:** Dual-Meter Types, Free-Tier Auto-Detect, Channel-Awareness

**Published:** June 2026
**NPM Package:** `@soilsidekick/sdk@3.0.0`

### What's New

- **Dual-meter usage types** — `DualMeterUsage` splits `daily_ai_count` and `daily_data_count` (plus `alerts_used`/`alerts_limit`) so clients can render AI vs. data quota separately.
- **Free-tier auto-detect** — SDK auto-provisions a free-tier key when none is supplied; hardcoded `ak_...` keys are no longer required for free endpoints.
- **Channel-aware config** — `ChannelAwareConfig` tags requests with a channel (`web`, `qgis`, `telegram`, `mcp`, `n8n`, `node-red`) for correct attribution in usage analytics.
- **Companion releases** — QGIS plugin `qgis_leafengines` `1.0.10` (security-clean, WFS `x-api-key` auth) and `@ancientwhispers54/leafengines-telemetry` `2.0.0` (dual-meter events).

### Breaking Changes (2.0 → 3.0)

| Change | Migration Path |
|---|---|
| `daily_call_count` split into `daily_ai_count` + `daily_data_count` | SDK auto-detects; legacy field maps to `daily_ai_count`. |
| Free-tier no longer requires `ak_` key | Remove hardcoded API key from config; SDK auto-provisions. |

Full breaking-change window: deprecated on release, removal 30 days later. See [SDK v3 Track](./docs/sdk/SDK_v3_TRACK.md) and [SDK v3 Generation Guide](./docs/sdk/SDK_v3_GENERATION_GUIDE.md).
...
```bash
# NPM
npm install @soilsidekick/sdk@3.0.0
