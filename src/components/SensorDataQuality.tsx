...
import { DEMO_SENSOR_QUALITY } from '@/lib/demoFixtures';
import { DemoDataBadge } from '@/components/DemoDataWatermark';
...
export function SensorDataQuality() {
  ...
  useEffect(() => {
    ...
    async function fetchLatestQuality() {
      try {
        // Guard against a request that never settles — always resolve the panel.
        ...
        const timeout = new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error('Sensor quality request timed out')), 10000)
        );

        const { data, error } = (await Promise.race([query, timeout])) as Awaited<typeof query>;
        if (cancelled) return;

        const rows = (data ?? []) as unknown as SensorQualityRecord[];

        // Demo mode: fixtures only when the real query is empty (or blocked),
        // and they render through this exact same grading/alert path.
        if (isDemoMode() && rows.length === 0) {
          setRecords(DEMO_SENSOR_QUALITY as unknown as SensorQualityRecord[]);
        } else if (error) {
        ...
        if (!cancelled) {
          setLoadError(err instanceof Error ? err.message : 'Failed to load sensor quality data');
        }
      } finally {
      ...
      <Card>
        ...
        <CardContent>
          <Alert variant="destructive">
            ...
            <AlertTitle>Couldn't load sensor quality</AlertTitle>
    ...
    return (
      <Card>
        ...
        <CardContent className="space-y-2">
          <p className="font-medium">Connect a sensor to see data quality</p>
          <p className="text-sm text-muted-foreground">
            Once a sensor reports readings, this panel shows drift detection, calibration staleness and an A–F confidence grade for each device.
          </p>
        </CardContent>
      </Card>
    );
