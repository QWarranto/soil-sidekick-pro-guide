...
/**
 * Rows store county_name as "Houston County, Georgia" — strip the trailing
 * state so labels don't read "Houston County, Georgia, Georgia".
 */
const cleanCountyName = (county: County): string => {
  const name = county.county_name ?? '';
  const suffixes = [county.state_name, county.state_code].filter(Boolean) as string[];
...
export const CountyLookup: React.FC<CountyLookupProps> = ({ onCountySelect }) => {
  ...
  const handleCountySelect = (county: County) => {
    const normalized: County = { ...county, county_name: cleanCountyName(county) };
    const label = `${normalized.county_name}, ${normalized.state_code}`;
    setSelectedCounty(normalized);
    setSelectedLabel(label);
    setSearchTerm(label);
    setCounties([]);
    onCountySelect(normalized);
  };

  useEffect(() => {
    // Don't re-query for the label we just wrote into the input after a
    // selection — that lookup returns nothing and flashed "No counties found"
    // underneath the selected county.
    if (selectedLabel && searchTerm === selectedLabel) return;
  ...
  return (
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
            ...
            {counties.map((county) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium">{cleanCountyName(county)}</p>
        ...
        {searchTerm.length > 0 && counties.length === 0 && !loading && !selectedCounty && (
