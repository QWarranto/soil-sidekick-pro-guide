...
import { DEMO_FIELDS, DEMO_SOIL_ANALYSES } from '@/lib/demoFixtures';
...
export const useFields = () => {
  ...
  const { data: fields = [], isLoading, error } = useQuery({
    queryKey: ['fields', user?.id, demoMode],
    queryFn: async () => {
      if (!user?.id && !demoMode) throw new Error('User not authenticated');

      // Fetch fields
      let fieldsData: any[] = [];
      let soilAnalysesData: any[] = [];

      if (user?.id) {
        const { data: fData, error: fieldsError } = await supabase
          .from('fields')
          .select('*')
          .order('name');

        if (fieldsError && !demoMode) throw fieldsError;
        fieldsData = fData ?? [];

        // Fetch soil analyses for the user
        const { data: sData, error: soilError } = await supabase
          .from('soil_analyses')
          .select('*')
          .eq('user_id', user.id)
          .order('created_at', { ascending: false });

        if (soilError && !demoMode) throw soilError;
        soilAnalysesData = sData ?? [];
      }

      // Demo mode is INERT whenever real data exists — fixtures only fill a
      // genuinely empty result, and then flow through the same merge/KPI path.
      if (demoMode && fieldsData.length === 0) {
        fieldsData = DEMO_FIELDS as any[];
        soilAnalysesData = DEMO_SOIL_ANALYSES as any[];
      }
