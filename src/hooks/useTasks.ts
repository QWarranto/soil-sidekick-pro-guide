...
import { DEMO_TASKS } from '@/lib/demoFixtures';
...
export const useTasks = () => {
  ...
  const { data: tasks = [], isLoading, error } = useQuery({
    queryKey: ['tasks', user?.id, demoMode],
    queryFn: async () => {
      ...
      // Inert when real tasks exist; fixtures only fill an empty result.
      if (demoMode && rows.length === 0) {
        rows = DEMO_TASKS as any[];
      }
