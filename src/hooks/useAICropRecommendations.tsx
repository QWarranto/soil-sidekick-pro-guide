...
export const useAICropRecommendations = () => {
  ...
  const getCropRecommendations = useCallback(async (
    ...
    try {
      ...
      // In demo mode the cached/SAMPLE DATA fallback is by-design (signed out),
      // so a red "AI service unavailable" toast would read as a real outage in
      // exhibit captures. The badge + "Source: Cached Data" already convey it.
      if (!isDemoMode()) {
        toast({
...
function extractNumericValue(text: string, pattern: string, min: number, max: number): number {
  ...
  // Deterministic fallback: hashed from the pattern + text so repeat renders
  // (and screenshot exhibits) produce identical values instead of drifting.
  return min + (stableHash(`${pattern}|${text.slice(0, 200)}`) % 1000) / 1000 * (max - min);
}
...
function generateFallbackRecommendations(requestedCrops?: string[]): CropRecommendationResponse {
  ...
  const recommendations: CropRecommendation[] = defaultCrops.map((crop, index) => ({
    ...
    suitability_score: [88, 92, 84, 79][index] ?? 82,
