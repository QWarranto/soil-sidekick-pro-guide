...
// Helper functions for default data — deterministic so repeat renders and
// screenshot exhibits are reproducible.
function generateDefaultForecast() {
  const hours = ['6AM', '9AM', '12PM', '3PM', '6PM', '9PM'];
  return hours.map((time, index) => ({
    time,
    temperature: 18 + index * 2 + [1.2, 2.4, 3.1, 0.6, 2.9, 1.7][index],
    humidity: 75 - index * 3 + [4, 7, 2, 9, 5, 3][index]
  }));
}
...
function generateDefaultSoilTrends() {
  ...
  return months.map((month, index) => ({
    month,
    health: 78 + index * 2 + [1, 3, 2, 4, 1, 3][index]
  }));
}