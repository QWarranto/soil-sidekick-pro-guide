...
LeafEngines plant compatibility API
...
```php
leafenginesQuery($leafengines_query_request, $x_tq_context_mode, $x_tq_kv_cache_hint, $x_tq_model_tier): \SoilSidekick\Model\LeafEnginesCompatibility
```
...
try {
    $result = $apiInstance->leafenginesQuery($leafengines_query_request, $x_tq_context_mode, $x_tq_kv_cache_hint, $x_tq_model_tier);
    print_r($result);
} catch (Exception $e) {
