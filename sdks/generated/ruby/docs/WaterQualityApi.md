...
> <WaterQuality> get_water_quality(get_soil_data_request)
...
begin
  # Get water quality data
  result = api_instance.get_water_quality(get_soil_data_request)
  p result
rescue SoilSidekick::ApiError => e
...
> <Array(<WaterQuality>, Integer, Hash)> get_water_quality_with_http_info(get_soil_data_request)

```ruby
begin
  # Get water quality data
  data, status_code, headers = api_instance.get_water_quality_with_http_info(get_soil_data_request)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <WaterQuality>
rescue SoilSidekick::ApiError => e
