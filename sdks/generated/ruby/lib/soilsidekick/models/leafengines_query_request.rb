...
module SoilSidekick
  class LeafenginesQueryRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SoilSidekick::LeafenginesQueryRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SoilSidekick::LeafenginesQueryRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @plant.nil?
        invalid_properties.push('invalid value for "plant", plant cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] plant Value to be assigned
    def plant=(plant)
      if plant.nil?
        fail ArgumentError, 'plant cannot be nil'
      end
