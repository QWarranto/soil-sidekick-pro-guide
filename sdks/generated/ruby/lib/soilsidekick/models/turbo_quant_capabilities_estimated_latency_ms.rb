...
module SoilSidekick
  class TurboQuantCapabilitiesEstimatedLatencyMs < ApiModelBase
    attr_accessor :first_token

    attr_accessor :per_token
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SoilSidekick::TurboQuantCapabilitiesEstimatedLatencyMs` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SoilSidekick::TurboQuantCapabilitiesEstimatedLatencyMs`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'first_token')
        self.first_token = attributes[:'first_token']
      end

      if attributes.key?(:'per_token')
        self.per_token = attributes[:'per_token']
      end
    end
