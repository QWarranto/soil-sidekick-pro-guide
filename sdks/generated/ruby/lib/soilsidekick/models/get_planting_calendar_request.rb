...
module SoilSidekick
  class GetPlantingCalendarRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SoilSidekick::GetPlantingCalendarRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SoilSidekick::GetPlantingCalendarRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @crop_type.nil?
        invalid_properties.push('invalid value for "crop_type", crop_type cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] crop_type Value to be assigned
    def crop_type=(crop_type)
      if crop_type.nil?
        fail ArgumentError, 'crop_type cannot be nil'
      end
