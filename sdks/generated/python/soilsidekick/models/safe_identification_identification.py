...
class SafeIdentificationIdentification(BaseModel):
    """
    SafeIdentificationIdentification
    """ # noqa: E501
    primary_match: Optional[SafeIdentificationIdentificationPrimaryMatch] = None
    environmental_probability: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Likelihood this plant exists in the given environment")
    growth_stage_detected: Optional[StrictStr] = None
    __properties: ClassVar[List[str]] = ["primary_match", "environmental_probability", "growth_stage_detected"]

    @field_validator('growth_stage_detected')
    def growth_stage_detected_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['seedling', 'juvenile', 'mature', 'flowering', 'fruiting']):
            raise ValueError("must be one of enum values ('seedling', 'juvenile', 'mature', 'flowering', 'fruiting')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of primary_match
        if self.primary_match:
            _dict['primary_match'] = self.primary_match.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "primary_match": SafeIdentificationIdentificationPrimaryMatch.from_dict(obj["primary_match"]) if obj.get("primary_match") is not None else None,
            "environmental_probability": obj.get("environmental_probability"),
            "growth_stage_detected": obj.get("growth_stage_detected")
        })
        return _obj
