...
class VisualCropAnalysisRequest(BaseModel):
    """
    VisualCropAnalysisRequest
    """ # noqa: E501
    image: StrictStr = Field(description="Base64 encoded image or image URL")
    analysis_type: StrictStr
    location: Optional[VisualCropAnalysisRequestLocation] = None
    crop_type: Optional[StrictStr] = Field(default=None, description="Type of crop being analyzed")
    __properties: ClassVar[List[str]] = ["image", "analysis_type", "location", "crop_type"]

    @field_validator('analysis_type')
    def analysis_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['pest_detection', 'crop_health', 'disease_screening']):
            raise ValueError("must be one of enum values ('pest_detection', 'crop_health', 'disease_screening')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "location": VisualCropAnalysisRequestLocation.from_dict(obj["location"]) if obj.get("location") is not None else None,
