...
class GenerateSmartReportSummaryRequest(BaseModel):
    """
    GenerateSmartReportSummaryRequest
    """ # noqa: E501
    report_type: StrictStr = Field(alias="reportType")
    report_data: Dict[str, Any] = Field(description="Report data to summarize", alias="reportData")
    __properties: ClassVar[List[str]] = ["reportType", "reportData"]

    @field_validator('report_type')
    def report_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['soil', 'water_quality']):
            raise ValueError("must be one of enum values ('soil', 'water_quality')")
        return value
