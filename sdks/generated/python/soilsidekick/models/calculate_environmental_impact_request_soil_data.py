...
class CalculateEnvironmentalImpactRequestSoilData(BaseModel):
    """
    CalculateEnvironmentalImpactRequestSoilData
    """ # noqa: E501
    ph_level: Optional[Union[StrictFloat, StrictInt]] = None
    organic_matter: Optional[Union[StrictFloat, StrictInt]] = None
    slope: Optional[StrictStr] = None
    drainage: Optional[StrictStr] = None
    nitrogen_level: Optional[StrictStr] = None
    phosphorus_level: Optional[StrictStr] = None
    __properties: ClassVar[List[str]] = ["ph_level", "organic_matter", "slope", "drainage", "nitrogen_level", "phosphorus_level"]

    @field_validator('slope')
    def slope_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['flat', 'gentle', 'moderate', 'steep', 'very_steep']):
            raise ValueError("must be one of enum values ('flat', 'gentle', 'moderate', 'steep', 'very_steep')")
        return value

    @field_validator('drainage')
    def drainage_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['poor', 'moderate', 'good', 'excessive']):
            raise ValueError("must be one of enum values ('poor', 'moderate', 'good', 'excessive')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "ph_level": obj.get("ph_level"),
            "organic_matter": obj.get("organic_matter"),
            "slope": obj.get("slope"),
            "drainage": obj.get("drainage"),
            "nitrogen_level": obj.get("nitrogen_level"),
            "phosphorus_level": obj.get("phosphorus_level")
        })
        return _obj
