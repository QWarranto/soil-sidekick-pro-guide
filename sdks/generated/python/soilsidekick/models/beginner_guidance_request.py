...
class BeginnerGuidanceRequest(BaseModel):
    """
    BeginnerGuidanceRequest
    """ # noqa: E501
    question: StrictStr = Field(description="User's plant question in natural language", json_schema_extra={"examples": ["My plant has yellow leaves, what's wrong?"]})
    plant_context: Optional[BeginnerGuidanceRequestPlantContext] = None
    location: Optional[BeginnerGuidanceRequestLocation] = None
    user_expertise: Optional[StrictStr] = Field(default='complete_beginner', description="User's self-assessed expertise level")
    __properties: ClassVar[List[str]] = ["question", "plant_context", "location", "user_expertise"]

    @field_validator('user_expertise')
    def user_expertise_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['complete_beginner', 'some_experience', 'intermediate']):
            raise ValueError("must be one of enum values ('complete_beginner', 'some_experience', 'intermediate')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of plant_context
        if self.plant_context:
            _dict['plant_context'] = self.plant_context.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "plant_context": BeginnerGuidanceRequestPlantContext.from_dict(obj["plant_context"]) if obj.get("plant_context") is not None else None,
            "location": BeginnerGuidanceRequestLocation.from_dict(obj["location"]) if obj.get("location") is not None else None,
            "user_expertise": obj.get("user_expertise") if obj.get("user_expertise") is not None else 'complete_beginner'
        })
        return _obj
