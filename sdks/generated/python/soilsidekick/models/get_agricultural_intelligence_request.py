...
class GetAgriculturalIntelligenceRequest(BaseModel):
    """
    GetAgriculturalIntelligenceRequest
    """ # noqa: E501
    county_fips: Annotated[str, Field(strict=True)]
    analysis_type: StrictStr
    __properties: ClassVar[List[str]] = ["county_fips", "analysis_type"]
    ...
    def analysis_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['crop_recommendation', 'yield_prediction', 'pest_analysis']):
            raise ValueError("must be one of enum values ('crop_recommendation', 'yield_prediction', 'pest_analysis')")
        return value
