...
class SafeIdentificationSafetyAnalysis(BaseModel):
    """
    SafeIdentificationSafetyAnalysis
    """ # noqa: E501
    toxicity_level: Optional[StrictStr] = None
    toxic_to: Optional[List[StrictStr]] = Field(default=None, description="List of animals/people this is toxic to (e.g., cats, dogs, children)")
    lookalikes: Optional[List[SafeIdentificationSafetyAnalysisLookalikesInner]] = None
    warnings: Optional[List[StrictStr]] = None
    __properties: ClassVar[List[str]] = ["toxicity_level", "toxic_to", "lookalikes", "warnings"]

    @field_validator('toxicity_level')
    def toxicity_level_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['safe', 'mildly_toxic', 'toxic', 'highly_toxic']):
            raise ValueError("must be one of enum values ('safe', 'mildly_toxic', 'toxic', 'highly_toxic')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in lookalikes (list)
        _items = []
        if self.lookalikes:
            for _item_lookalikes in self.lookalikes:
                if _item_lookalikes:
                    _items.append(_item_lookalikes.to_dict())
            _dict['lookalikes'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "toxic_to": obj.get("toxic_to"),
            "lookalikes": [SafeIdentificationSafetyAnalysisLookalikesInner.from_dict(_item) for _item in obj["lookalikes"]] if obj.get("lookalikes") is not None else None,
