...
> 🆕 **Now on v1.0.10 — repository security block cleared.** Builds on v1.0.6's end-to-end WFS (single `x-api-key` auth, `sc:managed_assets` namespace, green Connection Test, live `FeatureCollection` responses) and adds the v1.0.7→v1.0.10 hardening track: Bandit B411 fully eliminated by removing `xml.etree.ElementTree` from `wfs_connection.py`, runtime `QUrl`/`QNetworkRequest` import fixes, and a clean repository scan. Full scorecard and migration notes: **[QGIS Plugin Changelog →](/docs/workflows/qgis-plugin-changelog)**.
...
> **Companion docs:** [12 QGIS SDK Deep Dive](12_QGIS_SDK_DEEP_DIVE.md) · [SoilCertify GIS Resources](soilcertify/TECHNICAL_OVERVIEW.md) · [Plugin README](../../plugins/qgis-leafengines/README.md) · [WFS Extension](../../plugins/qgis-leafengines/WFS_EXTENSION.md) · [MCP Specification](../MCP_SERVER_SPECIFICATION.md)
...
- **GIS consultant resources (SoilCertify):** [Technical Overview](soilcertify/TECHNICAL_OVERVIEW.md) · [Developer Reference](soilcertify/DEVELOPER_LANDING.md) · [Infrastructure Use Case](soilcertify/INFRASTRUCTURE_USE_CASE.md)
