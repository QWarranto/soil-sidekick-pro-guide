...
-- Drop existing overlapping policies on account_security
...
CREATE POLICY "Users can insert own security data"
ON public.account_security
FOR INSERT
...
-- Keep admin and service role access policies
-- Note: Verified admins and service role policies remain unchanged

-- Security Fix: Upgrade trigger to BLOCK plaintext writes to subscribers table
-- Previously it only warned, now it will prevent insecure operations
...
BEGIN
  ...
  IF NEW.email IS NOT NULL OR NEW.stripe_customer_id IS NOT NULL THEN
    RAISE EXCEPTION 'Direct writes to plaintext columns are prohibited. Use secure_upsert_subscriber() function instead.';
  END IF;
...
-- Security Fix: Add input validation to critical SECURITY DEFINER functions
-- Update handle_login_attempt to validate email input
...
BEGIN
    ...
    IF attempt_success THEN
        ...
        INSERT INTO public.auth_security_log (
            ...
            user_record.id, validated_email, 'login_success', client_ip, user_agent_string,
    ...
    ELSE
        ...
        IF security_record.failed_login_attempts + 1 >= max_attempts THEN
            ...
            INSERT INTO public.auth_security_log (
                ...
                user_record.id, validated_email, 'account_locked', client_ip, user_agent_string,
        ...
        INSERT INTO public.auth_security_log (
            ...
            user_record.id, validated_email, 'login_fail', client_ip, user_agent_string,
...
-- Update secure_upsert_subscriber to add numeric validation
...
INSERT INTO public.comprehensive_audit_log (
    ...
    NULL,
    'CRITICAL',
    ARRAY['SECURITY_FIX', 'RLS_CONSOLIDATION', 'INPUT_VALIDATION', 'ENCRYPTION'],
    jsonb_build_object(
        'timestamp', now(),
        'fixes_applied', ARRAY[
            'Consolidated account_security RLS policies',
            'Upgraded prevent_plaintext_payment_data to block writes',
            'Added email validation to handle_login_attempt',
            'Added email validation to secure_upsert_subscriber'
        ],
        'security_impact', 'HIGH',
        'compliance_improvement', 'Addresses XSS, RLS, and encryption security findings'
    )
);