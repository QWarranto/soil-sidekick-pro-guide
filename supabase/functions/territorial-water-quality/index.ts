...
const FREE_TIER_USER = { id: '00000000-0000-0000-0000-000000000000', subscription_tier: 'free', free_tier: true };
...
async function authenticateUser(supabase: any, request: Request): Promise<{ user: any; error?: string }> {
  try {
    ...
    // Keyless / public callers (Telegram bot, social tools, anon publishable key)
    // get free-tier access instead of a 401.
    const anonKey = Deno.env.get('SUPABASE_ANON_KEY');
    if (request.headers.get('x-free-tier') === 'true' || !token || token === anonKey) {
      console.log('Free-tier access granted (public/keyless caller)');
      return { user: FREE_TIER_USER };
    }

    const { data: { user }, error } = await supabase.auth.getUser(token);

    if (error || !user) {
      // No API key was presented, so degrade to free tier rather than reject.
...
const corsHeaders = {
  ...
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type, x-api-key, x-free-tier',
}
