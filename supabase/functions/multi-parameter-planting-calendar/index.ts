...
requestHandler({
  ...
  useServiceRole: true,
  handler: async (ctx) => {
    const { supabaseClient: supabase, validatedData, req } = ctx as any;
    const { county_fips, crop_type, soil_data = {}, climate_preferences = {}, sustainability_goals = [] } = validatedData as any;

    // Optional auth: API keys (ak_*) are validated strictly; keyless/anon callers
    // are served as free tier (public tool used by Telegram bot / social funnels).
    const bearer = (req?.headers?.get('authorization') ?? '').replace(/^Bearer\s+/i, '');
    const providedKey = req?.headers?.get('x-api-key') || (bearer.startsWith('ak_') ? bearer : '');
    let user: any = null;
    try {
    ...
    if (!user && providedKey.startsWith('ak_')) {
      return new Response(
    ...
    // Track cost for climate API call (authenticated users only)
    if (!isFreeTier) {
    ...
    // Store optimization results (skipped for keyless free-tier callers)
    let optimization: { id?: string } | null = null;
    if (!isFreeTier) {
...
function getFrostDates(region: string): any {
  // Month-day templates; the calendar year is resolved against today so the
  // returned planting windows are always in the upcoming season, never the past.
  const dates: Record<string, { last: string; first: string }> = {
    'southeast': { last: '03-15', first: '11-15' },
    'southwest': { last: '02-15', first: '12-01' },
    'pacific': { last: '02-28', first: '11-30' },
    'mountain': { last: '05-15', first: '09-15' },
    'midwest': { last: '04-20', first: '10-15' },
    'northeast': { last: '04-30', first: '10-01' },
    'plains': { last: '04-25', first: '10-10' },
    'south_central': { last: '03-20', first: '11-10' },
    'temperate': { last: '04-15', first: '10-20' }
  };
  const tmpl = dates[region] || dates['temperate'];
  const today = new Date();
  let year = today.getUTCFullYear();
  // If this season's last-frost date has already passed, target next season.
  if (new Date(`${year}-${tmpl.last}T00:00:00Z`) < today) {
    year += 1;
  }
  return { last: `${year}-${tmpl.last}`, first: `${year}-${tmpl.first}` };
}
