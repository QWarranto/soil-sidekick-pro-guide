...
// Classifies the origin of an MCP call so we can measure adoption per
// channel (GitHub MCP Registry, OpenClaw, Composio, Cursor, direct API).
// Precedence: explicit `x-mcp-source` header (canonical) > User-Agent sniff.
const KNOWN_SOURCES = new Set([
  'github-mcp-registry', 'claude-desktop', 'cursor', 'openclaw',
  'composio', 'n8n', 'node-red', 'qgis', 'telegram', 'direct-api', 'unknown',
]);
function detectSource(headers: Headers): string {
  const explicit = (headers.get('x-mcp-source') || '').trim().toLowerCase();
  if (explicit && KNOWN_SOURCES.has(explicit)) return explicit;
  const ua = (headers.get('user-agent') || '').toLowerCase();
  if (!ua) return 'unknown';
  if (ua.includes('github-mcp') || ua.includes('mcp-registry')) return 'github-mcp-registry';
  if (ua.includes('claude')) return 'claude-desktop';
  if (ua.includes('cursor')) return 'cursor';
  if (ua.includes('openclaw') || ua.includes('clawhub')) return 'openclaw';
  if (ua.includes('composio')) return 'composio';
  if (ua.includes('n8n')) return 'n8n';
  if (ua.includes('node-red')) return 'node-red';
  if (ua.includes('qgis')) return 'qgis';
...
const TOOL_ALIASES: Record<string, string> = {
  ...
  // Agent probe patterns observed in July 6 + July 17 telemetry
  crop_recommendation: 'agricultural_intelligence',
  crop_recommendations: 'agricultural_intelligence',
  recommend_crop: 'agricultural_intelligence',
  yield_prediction: 'agricultural_intelligence',
};
...
interface ReqMeta { ip?: string; userAgent?: string; isBatch?: boolean; correlationId?: string; source?: string; clientInstanceId?: string; sdk?: string; }
...
async function handleRpc(req: JsonRpcRequest, apiKey: string | null, reqMeta?: ReqMeta): Promise<unknown> {
  ...
  if (method === 'tools/call') {
    ...
    if (FIPS_REQUIRED_TOOLS.has(toolName)) {
      ...
      if (!fips || typeof fips !== 'string' || !/^\d{5}$/.test(fips)) {
        ...
        logToolCall({
          ...
          access_source: reqMeta?.source,
          client_instance_id: reqMeta?.clientInstanceId,
          sdk: reqMeta?.sdk,
          success: false,
          response_status: 400,
    ...
    try {
      ...
      // Hard-cap downstream latency so a stalled upstream (e.g. SSURGO) can't
      // sit near the edge-runtime ceiling. Observed 70.4s on get_soil_data
      // (2026-07-06 09:39 UTC) — bound to 20s and surface a clean 504.
      ...
      try {
        ...
        const errMsg = aborted
          ? `Upstream ${endpoint} exceeded ${UPSTREAM_TIMEOUT_MS}ms; request aborted.`
          : `Upstream ${endpoint} fetch failed: ${(fetchErr as Error)?.message ?? 'unknown'}`;
        logToolCall({
          ...auditBase, success: false, error_message: errMsg,
          response_status: aborted ? 504 : 502, response_time_ms: elapsed,
          downstream_endpoint: endpoint,
        });
        return jsonRpcResponse(id ?? null, {
          content: [{ type: 'text', text: aborted
            ? `Timeout: the ${toolName} data source did not respond within ${UPSTREAM_TIMEOUT_MS / 1000}s. Try again shortly — for cached counties this usually clears on retry.`
            : `Upstream error contacting ${toolName}. Please retry.` }],
          isError: true,
        });
      }
      clearTimeout(timeoutId);
...
Deno.serve(async (req) => {
  ...
  const meta: ReqMeta = {
    ...
    clientInstanceId: (req.headers.get('x-client-instance-id') || '').trim().slice(0, 128) || undefined,
    sdk: (req.headers.get('x-sdk') || req.headers.get('x-sdk-version') || '').trim().slice(0, 64) || undefined,
  };
  ...
  if (req.method === 'OPTIONS') {
    return new Response(null, {
      headers: {
        ...corsHeaders,
        'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type, x-api-key, accept, x-mcp-source, x-client-instance-id, x-sdk, x-sdk-version, x-correlation-id',
      },
    });
  }
