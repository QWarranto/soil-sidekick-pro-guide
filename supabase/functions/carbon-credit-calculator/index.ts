/**
 * Carbon Credit Calculator Edge Function
 * Migrated to requestHandler: December 12, 2025 (Phase 4A.2)
 * 
 * Features:
 * - Zod validation with carbonCreditSchema
 * - Professional tier subscription required
 * - Rate limiting: 100 requests/hour
 * - Cost tracking for calculations
 * - Compliance logging for carbon credit transactions
 */

import { requestHandler } from '../_shared/request-handler.ts';
import { carbonCreditSchema } from '../_shared/validation.ts';
...
requestHandler({
  ...
  validationSchema: carbonCreditSchema,
  rateLimit: {
    requests: 100,
  ...
  handler: async ({ supabaseClient, user, validatedData, startTime }) => {
    ...
    const { data: creditRecord, error: insertError } = await supabaseClient
      .from('carbon_credits')
      .insert({
        user_id: user.id,
        field_name: validatedData.field_name,
        field_size_acres: validatedData.field_size_acres,
        soil_organic_matter: validatedData.soil_organic_matter,
        credits_earned: calculation.credits_earned,
        verification_status: validatedData.verification_type === 'third_party' ? 'verified' : 'pending',
    ...
    if (insertError) {
      ...
      throw new Error('Failed to store carbon credit calculation');
    }

    // Log compliance audit for carbon credit creation
    await logComplianceAudit(supabaseClient, {
      table_name: 'carbon_credits',
      operation: 'CARBON_CREDIT_CALCULATION',
      user_id: user.id,
      ...
      risk_level: 'medium',
      compliance_tags: ['CARBON_CREDIT', 'ENVIRONMENTAL', 'AUDIT_TRAIL'],
      metadata: {
        calculation_method: calculation.calculation_method,
        verification_confidence: calculation.verification_confidence,
      },
    });

    // Track cost for this calculation
    await trackCost(supabaseClient, {
      service_provider: 'internal',
      service_type: 'carbon_credit_calculation',
      cost_usd: 0.01, // Minimal internal cost for calculation
      feature_name: 'carbon-credit-calculator',
      user_id: user.id,
      request_details: {
        field_size_acres: validatedData.field_size_acres,
        credits_earned: calculation.credits_earned,
...
async function calculateCarbonCredits(
  ...
  // Adjust confidence based on verification type
  if (request.verification_type === 'third_party') {
    confidenceScore = Math.min(confidenceScore + 0.15, 1.0);
    dataSources.push('third_party_verification');
  } else if (request.verification_type === 'satellite') {
    confidenceScore = Math.min(confidenceScore + 0.1, 0.95);
    dataSources.push('satellite_imagery');
  }
  ...
  const carbonPerHectare =
    carbonIncrease * bulkDensity * depthCm * 10000 * carbonFraction * kgToTonne; // tonnes C/ha
