...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const incomingKey = xApiKeyHeader || bearerToken;
    const anonKey = (Deno.env.get('SUPABASE_ANON_KEY') || '').trim();
    const hasCustomApiKey = xApiKeyHeader.startsWith('ak_') || bearerToken.startsWith('ak_');
    // Keyless callers (Telegram bot, public tools) only present the publishable
    // anon key — or nothing at all. Treat those as free tier instead of 401.
    const isKeylessPublicCaller = !hasCustomApiKey
      && (!bearerToken || (anonKey && bearerToken === anonKey));
    const isFreeTier = req.headers.get('x-free-tier') === 'true'
      || PUBLIC_FREE_TIER_KEYS.has(incomingKey)
      || isKeylessPublicCaller;
    ...
    if (!isFreeTier) {
      ...
      if (authResult.error || !authResult.user) {
        if (!hasCustomApiKey) {
          // Non-ak_ credential that isn't a valid user JWT (expired session, bot
          // caller sending a service token) — serve public free-tier data instead of 401.
          console.log('Auth failed for non-API-key caller; degrading to free tier');
          degradedToFreeTier = true;
        } else {
          logResponseTime(ENDPOINT_NAME, startTime, false);
          return new Response(
    ...
    if (isFreeTier || degradedToFreeTier) {
      console.log('Free-tier access granted (public/keyless caller)');
