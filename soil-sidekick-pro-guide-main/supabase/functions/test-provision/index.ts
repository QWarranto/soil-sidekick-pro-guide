...
serve(async (req) => {
  ...
  try {
    ...
    errors.push(`1. Existing: ${existing ? 'yes, tier=' + (existing.api_keys as any)?.subscription_tier : 'no'}`);
    ...
    if (!existing) {
      ...
      if (apiKey?.id) {
        ...
        if (!linkError) {
          ...
          errors.push('4. Cleaned up telegram_link');
        }
        await supabase.from('api_keys').delete().eq('id', apiKey.id);
        errors.push('5. Cleaned up api_key');
      }
    }
