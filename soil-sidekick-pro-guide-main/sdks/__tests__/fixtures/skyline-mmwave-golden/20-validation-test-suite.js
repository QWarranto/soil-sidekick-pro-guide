...
class SkylineFixtureValidator {
  ...
  validateFixture(fixturePath) {
    ...
    if (fixtureName === '10-corrupt-timing-anomalies.json') {
      // This contains a readings array with two timing anomaly readings
      const fixtureData = JSON.parse(fs.readFileSync(fixturePath, 'utf8'));
      
      if (!fixtureData.readings || !Array.isArray(fixtureData.readings)) {
        return [{
          fixture: fixtureName,
          valid: false,
          errors: ['Missing or invalid readings array'],
          schemaVersion: null,
          expectedValid: false
        }];
      }
      ...
      const results = [];
      for (let i = 0; i < fixtureData.readings.length; i++) {
        const reading = fixtureData.readings[i];
        results.push(this.validateSingleFixture(reading, `${fixtureName} - reading ${i + 1}`));
      }
  ...
  validateSingleFixture(data, fixtureName) {
    ...
    const expectedValid = this.getExpectedValidity(fixtureName, schemaVersion);
    
    // For N/A fixtures (expectedValid === null), valid should also be null
  ...
  getExpectedValidity(fixtureName, schemaVersion) {
    ...
    if (fixtureName === '13-schema-migration-path-v1.0-v1.1-v2.0.json') {
      // Migration fixture contains v1.0, v1.1, and v2.0 data
      // Each should be valid against its respective schema
      return true;
    }
    ...
    // package.json files are not sensor data
    if (fixtureName === 'package.json' || fixtureName === 'package-lock.json') {
  ...
  validateAllFixtures() {
    ...
    // Get all JSON fixture files (exclude schemas and package files)
  ...
  generateValidationReport(results) {
    ...
    const naCount = results.filter(r => r.expectedValid === null).length;
