...
class GenerateVRTPrescriptionRequest(BaseModel):
    """
    GenerateVRTPrescriptionRequest
    """ # noqa: E501
    field_id: StrictStr
    application_type: StrictStr
    soil_analysis_id: Optional[StrictStr] = None
    __properties: ClassVar[List[str]] = ["field_id", "application_type", "soil_analysis_id"]

    @field_validator('application_type')
    def application_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('fertilizer', 'seed', 'pesticide'):
            raise ValueError("must be one of enum values ('fertilizer', 'seed', 'pesticide')")
        return value
