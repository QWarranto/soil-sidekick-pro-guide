...
class LeafenginesQueryRequestPlantCareRequirements(BaseModel):
    """
    LeafenginesQueryRequestPlantCareRequirements
    """ # noqa: E501
    sun_exposure: Optional[StrictStr] = None
    water_needs: Optional[StrictStr] = None
    soil_ph_range: Optional[LeafenginesQueryRequestPlantCareRequirementsSoilPhRange] = None
    __properties: ClassVar[List[str]] = ["sun_exposure", "water_needs", "soil_ph_range"]

    @field_validator('sun_exposure')
    def sun_exposure_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in ('full_sun', 'partial_shade', 'full_shade'):
            raise ValueError("must be one of enum values ('full_sun', 'partial_shade', 'full_shade')")
        return value

    @field_validator('water_needs')
    def water_needs_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of soil_ph_range
        if self.soil_ph_range:
            _dict['soil_ph_range'] = self.soil_ph_range.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            "sun_exposure": obj.get("sun_exposure"),
            "water_needs": obj.get("water_needs"),
            "soil_ph_range": LeafenginesQueryRequestPlantCareRequirementsSoilPhRange.from_dict(obj.get("soil_ph_range")) if obj.get("soil_ph_range") is not None else None
        })
        return _obj
