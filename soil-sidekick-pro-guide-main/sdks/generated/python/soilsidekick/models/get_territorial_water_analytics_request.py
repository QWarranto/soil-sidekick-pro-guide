...
class GetTerritorialWaterAnalyticsRequest(BaseModel):
    """
    GetTerritorialWaterAnalyticsRequest
    """ # noqa: E501
    territory_type: Optional[StrictStr] = Field(default=None, description="Type of territory to analyze")
    epa_region: Optional[StrictStr] = Field(default=None, description="EPA region identifier")
    date_range: Optional[GetTerritorialWaterAnalyticsRequestDateRange] = None
    __properties: ClassVar[List[str]] = ["territory_type", "epa_region", "date_range"]

    @field_validator('territory_type')
    def territory_type_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in ('state', 'territory', 'compact_state'):
            raise ValueError("must be one of enum values ('state', 'territory', 'compact_state')")
        return value
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            "territory_type": obj.get("territory_type"),
            "epa_region": obj.get("epa_region"),
            "date_range": GetTerritorialWaterAnalyticsRequestDateRange.from_dict(obj.get("date_range")) if obj.get("date_range") is not None else None
        })
        return _obj
