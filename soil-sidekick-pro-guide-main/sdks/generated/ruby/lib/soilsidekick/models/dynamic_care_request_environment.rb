...
module SoilSidekick
  class DynamicCareRequestEnvironment
    attr_accessor :light_exposure

    # Approximate indoor humidity if applicable
    attr_accessor :humidity_level
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SoilSidekick::DynamicCareRequestEnvironment` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SoilSidekick::DynamicCareRequestEnvironment`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'light_exposure')
        self.light_exposure = attributes[:'light_exposure']
      end

      if attributes.key?(:'humidity_level')
        self.humidity_level = attributes[:'humidity_level']
      end
    end
    ...
    def valid?
      ...
      light_exposure_validator = EnumAttributeValidator.new('String', ["full_sun", "partial_sun", "partial_shade", "full_shade", "artificial"])
      return false unless light_exposure_validator.valid?(@light_exposure)
      humidity_level_validator = EnumAttributeValidator.new('String', ["low", "medium", "high"])
      return false unless humidity_level_validator.valid?(@humidity_level)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] light_exposure Object to be assigned
    def light_exposure=(light_exposure)
      validator = EnumAttributeValidator.new('String', ["full_sun", "partial_sun", "partial_shade", "full_shade", "artificial"])
      unless validator.valid?(light_exposure)
        fail ArgumentError, "invalid value for \"light_exposure\", must be one of #{validator.allowable_values}."
      end
      @light_exposure = light_exposure
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] humidity_level Object to be assigned
    def humidity_level=(humidity_level)
      validator = EnumAttributeValidator.new('String', ["low", "medium", "high"])
      unless validator.valid?(humidity_level)
        fail ArgumentError, "invalid value for \"humidity_level\", must be one of #{validator.allowable_values}."
      end
      @humidity_level = humidity_level
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          light_exposure == o.light_exposure &&
          humidity_level == o.humidity_level
    end
    ...
    def hash
      [light_exposure, humidity_level].hash
    end
