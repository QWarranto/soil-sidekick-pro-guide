...
module SoilSidekick
  class VisualCropAnalysisRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SoilSidekick::VisualCropAnalysisRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SoilSidekick::VisualCropAnalysisRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      analysis_type_validator = EnumAttributeValidator.new('String', ["pest_detection", "crop_health", "disease_screening"])
    ...
    def analysis_type=(analysis_type)
      validator = EnumAttributeValidator.new('String', ["pest_detection", "crop_health", "disease_screening"])
    ...
    def hash
      [image, analysis_type, location, crop_type].hash
    end
