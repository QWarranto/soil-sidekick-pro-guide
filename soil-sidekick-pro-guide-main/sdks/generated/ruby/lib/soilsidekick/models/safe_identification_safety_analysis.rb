...
module SoilSidekick
  class SafeIdentificationSafetyAnalysis
    attr_accessor :toxicity_level

    # List of animals/people this is toxic to (e.g., cats, dogs, children)
    attr_accessor :toxic_to

    attr_accessor :lookalikes
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SoilSidekick::SafeIdentificationSafetyAnalysis` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SoilSidekick::SafeIdentificationSafetyAnalysis`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'toxicity_level')
        self.toxicity_level = attributes[:'toxicity_level']
      end

      if attributes.key?(:'toxic_to')
        if (value = attributes[:'toxic_to']).is_a?(Array)
          self.toxic_to = value
        end
      end

      if attributes.key?(:'lookalikes')
        if (value = attributes[:'lookalikes']).is_a?(Array)
          self.lookalikes = value
        end
      end
    ...
    def valid?
      ...
      toxicity_level_validator = EnumAttributeValidator.new('String', ["safe", "mildly_toxic", "toxic", "highly_toxic"])
      return false unless toxicity_level_validator.valid?(@toxicity_level)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] toxicity_level Object to be assigned
    def toxicity_level=(toxicity_level)
      validator = EnumAttributeValidator.new('String', ["safe", "mildly_toxic", "toxic", "highly_toxic"])
      unless validator.valid?(toxicity_level)
        fail ArgumentError, "invalid value for \"toxicity_level\", must be one of #{validator.allowable_values}."
      end
      @toxicity_level = toxicity_level
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          toxicity_level == o.toxicity_level &&
          toxic_to == o.toxic_to &&
          lookalikes == o.lookalikes &&
          warnings == o.warnings
    end
    ...
    def hash
      [toxicity_level, toxic_to, lookalikes, warnings].hash
    end
