...
module SoilSidekick
  class LeafEnginesCompatibilityDataBreakdown
    attr_accessor :soil
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SoilSidekick::LeafEnginesCompatibilityDataBreakdown` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SoilSidekick::LeafEnginesCompatibilityDataBreakdown`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'soil')
        self.soil = attributes[:'soil']
      end

      if attributes.key?(:'water')
        self.water = attributes[:'water']
      end

      if attributes.key?(:'climate')
        self.climate = attributes[:'climate']
      end
    end
