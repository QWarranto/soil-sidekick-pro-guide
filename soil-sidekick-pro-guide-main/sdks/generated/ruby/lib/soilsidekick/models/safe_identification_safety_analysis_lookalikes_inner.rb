...
module SoilSidekick
  class SafeIdentificationSafetyAnalysisLookalikesInner
    attr_accessor :plant_name

    # Visual similarity score (0-100)
    attr_accessor :visual_similarity

    attr_accessor :toxicity_level

    attr_accessor :distinguishing_features
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SoilSidekick::SafeIdentificationSafetyAnalysisLookalikesInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SoilSidekick::SafeIdentificationSafetyAnalysisLookalikesInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'plant_name')
        self.plant_name = attributes[:'plant_name']
      end

      if attributes.key?(:'visual_similarity')
        self.visual_similarity = attributes[:'visual_similarity']
      end
      ...
      if attributes.key?(:'distinguishing_features')
        if (value = attributes[:'distinguishing_features']).is_a?(Array)
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          visual_similarity == o.visual_similarity &&
          toxicity_level == o.toxicity_level &&
          distinguishing_features == o.distinguishing_features
    end
    ...
    def hash
      [plant_name, visual_similarity, toxicity_level, distinguishing_features].hash
    end
