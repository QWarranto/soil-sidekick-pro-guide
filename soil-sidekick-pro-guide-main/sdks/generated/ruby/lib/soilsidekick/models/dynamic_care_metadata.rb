...
module SoilSidekick
  class DynamicCareMetadata
    attr_accessor :location_data_used

    attr_accessor :weather_data_freshness
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SoilSidekick::DynamicCareMetadata` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SoilSidekick::DynamicCareMetadata`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'location_data_used')
        self.location_data_used = attributes[:'location_data_used']
      end

      if attributes.key?(:'weather_data_freshness')
        self.weather_data_freshness = attributes[:'weather_data_freshness']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          location_data_used == o.location_data_used &&
          weather_data_freshness == o.weather_data_freshness &&
    ...
    def hash
      [location_data_used, weather_data_freshness, confidence].hash
    end
