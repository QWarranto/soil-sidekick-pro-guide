...
module SoilSidekick
  class SafeIdentificationRequestLocation
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SoilSidekick::SafeIdentificationRequestLocation` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SoilSidekick::SafeIdentificationRequestLocation`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@county_fips.nil? && @county_fips !~ pattern
    ...
    def valid?
      ...
      return false if !@county_fips.nil? && @county_fips !~ Regexp.new(/^[0-9]{5}$/)
    ...
    def hash
      [county_fips, state_code, coordinates].hash
    end
