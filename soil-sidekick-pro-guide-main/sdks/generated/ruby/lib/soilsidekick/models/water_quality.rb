...
module SoilSidekick
  class WaterQuality
    ...
    attr_accessor :dissolved_oxygen

    attr_accessor :turbidity

    attr_accessor :nitrates

    attr_accessor :phosphates
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SoilSidekick::WaterQuality` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SoilSidekick::WaterQuality`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'dissolved_oxygen')
        self.dissolved_oxygen = attributes[:'dissolved_oxygen']
      end

      if attributes.key?(:'turbidity')
        self.turbidity = attributes[:'turbidity']
      end

      if attributes.key?(:'nitrates')
        self.nitrates = attributes[:'nitrates']
      end

      if attributes.key?(:'phosphates')
        self.phosphates = attributes[:'phosphates']
      end
    ...
    def valid?
      ...
      contamination_risk_validator = EnumAttributeValidator.new('String', ["low", "medium", "high"])
      return false unless contamination_risk_validator.valid?(@contamination_risk)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] contamination_risk Object to be assigned
    def contamination_risk=(contamination_risk)
      validator = EnumAttributeValidator.new('String', ["low", "medium", "high"])
      unless validator.valid?(contamination_risk)
        fail ArgumentError, "invalid value for \"contamination_risk\", must be one of #{validator.allowable_values}."
      end
      @contamination_risk = contamination_risk
    end
    ...
    def hash
      [county_fips, ph, dissolved_oxygen, turbidity, nitrates, phosphates, contamination_risk].hash
    end
