...
module SoilSidekick
  class GetTerritorialWaterAnalyticsRequest
    # Type of territory to analyze
    attr_accessor :territory_type

    # EPA region identifier
    attr_accessor :epa_region
    ...
    def self.openapi_types
      {
        ...
        :'epa_region' => :'String',
        :'date_range' => :'GetTerritorialWaterAnalyticsRequestDateRange'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SoilSidekick::GetTerritorialWaterAnalyticsRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SoilSidekick::GetTerritorialWaterAnalyticsRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'territory_type')
        self.territory_type = attributes[:'territory_type']
      end

      if attributes.key?(:'epa_region')
        self.epa_region = attributes[:'epa_region']
      end
    ...
    def valid?
      ...
      territory_type_validator = EnumAttributeValidator.new('String', ["state", "territory", "compact_state"])
      return false unless territory_type_validator.valid?(@territory_type)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] territory_type Object to be assigned
    def territory_type=(territory_type)
      validator = EnumAttributeValidator.new('String', ["state", "territory", "compact_state"])
      unless validator.valid?(territory_type)
        fail ArgumentError, "invalid value for \"territory_type\", must be one of #{validator.allowable_values}."
      end
      @territory_type = territory_type
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          territory_type == o.territory_type &&
          epa_region == o.epa_region &&
    ...
    def hash
      [territory_type, epa_region, date_range].hash
    end
