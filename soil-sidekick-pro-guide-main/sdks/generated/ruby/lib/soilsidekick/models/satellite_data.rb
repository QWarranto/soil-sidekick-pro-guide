  ...
  class SatelliteData
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SoilSidekick::SatelliteData` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SoilSidekick::SatelliteData`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'ndvi')
        self.ndvi = attributes[:'ndvi']
      end

      if attributes.key?(:'evi')
        self.evi = attributes[:'evi']
      end

      if attributes.key?(:'soil_moisture')
        self.soil_moisture = attributes[:'soil_moisture']
      end
      ...
      if attributes.key?(:'cloud_cover')
        self.cloud_cover = attributes[:'cloud_cover']
      end
    end
    ...
    def hash
      [ndvi, evi, soil_moisture, temperature, cloud_cover].hash
    end
