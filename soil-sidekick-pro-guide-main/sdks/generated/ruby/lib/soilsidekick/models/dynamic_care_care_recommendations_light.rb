...
module SoilSidekick
  class DynamicCareCareRecommendationsLight
    attr_accessor :current_assessment

    attr_accessor :adjustment_needed
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SoilSidekick::DynamicCareCareRecommendationsLight` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SoilSidekick::DynamicCareCareRecommendationsLight`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'current_assessment')
        self.current_assessment = attributes[:'current_assessment']
      end

      if attributes.key?(:'adjustment_needed')
        self.adjustment_needed = attributes[:'adjustment_needed']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          current_assessment == o.current_assessment &&
    ...
    def hash
      [current_assessment, adjustment_needed, recommendation].hash
    end
