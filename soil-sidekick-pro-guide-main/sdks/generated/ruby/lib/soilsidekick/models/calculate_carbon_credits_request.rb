...
module SoilSidekick
  class CalculateCarbonCreditsRequest
    # Name of the field
    attr_accessor :field_name

    # Field size in acres
    attr_accessor :field_size_acres

    # Soil organic matter percentage
    attr_accessor :soil_organic_matter

    # Reference to existing soil analysis
    attr_accessor :soil_analysis_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SoilSidekick::CalculateCarbonCreditsRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SoilSidekick::CalculateCarbonCreditsRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'field_size_acres')
        self.field_size_acres = attributes[:'field_size_acres']
      else
        self.field_size_acres = nil
      end

      if attributes.key?(:'soil_organic_matter')
        self.soil_organic_matter = attributes[:'soil_organic_matter']
      end

      if attributes.key?(:'soil_analysis_id')
        self.soil_analysis_id = attributes[:'soil_analysis_id']
      end
    ...
    def list_invalid_properties
      ...
      if @field_size_acres.nil?
        invalid_properties.push('invalid value for "field_size_acres", field_size_acres cannot be nil.')
      end

      if @field_size_acres < 0.1
        invalid_properties.push('invalid value for "field_size_acres", must be greater than or equal to 0.1.')
      end
    ...
    def valid?
      ...
      return false if @field_size_acres.nil?
      return false if @field_size_acres < 0.1
      verification_type_validator = EnumAttributeValidator.new('String', ["self_reported", "third_party", "satellite"])
      return false unless verification_type_validator.valid?(@verification_type)
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] field_size_acres Value to be assigned
    def field_size_acres=(field_size_acres)
      if field_size_acres.nil?
        fail ArgumentError, 'field_size_acres cannot be nil'
      end

      if field_size_acres < 0.1
        fail ArgumentError, 'invalid value for "field_size_acres", must be greater than or equal to 0.1.'
      end
    ...
    def verification_type=(verification_type)
      validator = EnumAttributeValidator.new('String', ["self_reported", "third_party", "satellite"])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          field_size_acres == o.field_size_acres &&
          soil_organic_matter == o.soil_organic_matter &&
          soil_analysis_id == o.soil_analysis_id &&
    ...
    def hash
      [field_name, field_size_acres, soil_organic_matter, soil_analysis_id, verification_type].hash
    end
