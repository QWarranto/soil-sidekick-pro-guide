  ...
  class CountyLookupRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SoilSidekick::CountyLookupRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SoilSidekick::CountyLookupRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @term.to_s.length > 100
        invalid_properties.push('invalid value for "term", the character length must be smaller than or equal to 100.')
      end

      if @term.to_s.length < 2
        invalid_properties.push('invalid value for "term", the character length must be great than or equal to 2.')
      end
    ...
    def valid?
      ...
      return false if @term.to_s.length > 100
      return false if @term.to_s.length < 2
      true
    end
    ...
    def term=(term)
      ...
      if term.to_s.length > 100
        fail ArgumentError, 'invalid value for "term", the character length must be smaller than or equal to 100.'
      end

      if term.to_s.length < 2
        fail ArgumentError, 'invalid value for "term", the character length must be great than or equal to 2.'
      end
