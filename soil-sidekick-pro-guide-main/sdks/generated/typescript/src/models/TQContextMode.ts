 ...
 * - standard: Default context window (~5 messages, 4K tokens)
 * - extended: TurboQuant-enabled extended context (~20 messages, 16K tokens)
 * - maximum: Maximum context with KV cache persistence (~30 messages, 24K tokens)
 * 
 * @export
 */
export const TQContextMode = {
    Standard: 'standard',
    Extended: 'extended',
    Maximum: 'maximum'
} as const;
export type TQContextMode = typeof TQContextMode[keyof typeof TQContextMode];


export function TQContextModeFromJSON(json: any): TQContextMode {
    return TQContextModeFromJSONTyped(json, false);
}

export function TQContextModeFromJSONTyped(json: any, ignoreDiscriminator: boolean): TQContextMode {
    return json as TQContextMode;
}

export function TQContextModeToJSON(value?: TQContextMode | null): any {
    return value as any;
}
