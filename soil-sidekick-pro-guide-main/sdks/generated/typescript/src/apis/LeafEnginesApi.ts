...
import {
    LeafEnginesCompatibilityFromJSON,
    LeafEnginesCompatibilityToJSON,
    LeafenginesQueryRequestFromJSON,
    LeafenginesQueryRequestToJSON,
} from '../models/index';
 ...
 * 
 */
export class LeafEnginesApi extends runtime.BaseAPI {

    /**
     * LeafEngines API for plant compatibility scoring with environmental factors
     * Query plant-environment compatibility
     */
    async leafenginesQueryRaw(requestParameters: LeafenginesQueryOperationRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<LeafEnginesCompatibility>> {
        if (requestParameters.leafenginesQueryRequest === null || requestParameters.leafenginesQueryRequest === undefined) {
            throw new runtime.RequiredError('leafenginesQueryRequest','Required parameter requestParameters.leafenginesQueryRequest was null or undefined when calling leafenginesQuery.');
        }
        ...
        const response = await this.request({
            ...
            body: LeafenginesQueryRequestToJSON(requestParameters.leafenginesQueryRequest),
        }, initOverrides);

        return new runtime.JSONApiResponse(response, (jsonValue) => LeafEnginesCompatibilityFromJSON(jsonValue));
    }
     ...
     * Query plant-environment compatibility
     */
    async leafenginesQuery(requestParameters: LeafenginesQueryOperationRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<LeafEnginesCompatibility> {
        const response = await this.leafenginesQueryRaw(requestParameters, initOverrides);
